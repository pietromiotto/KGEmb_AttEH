...
class KGModel(nn.Module, ABC):
    ...
    def get_factors(self, queries):
        """Compute factors for regularization."""
    ...
    def get_ranking(self, queries, filters, batch_size=1000):
        ...
        with torch.no_grad():
            ...
            while b_begin < len(queries):
                ...
                # Apply filtered setting
                for i, query in enumerate(these_queries):
                    filter_out = filters[(query[0].item(), query[1].item())]
                    filter_out.append(queries[b_begin + i, 2].item())
                    scores[i, torch.LongTensor(filter_out).to(scores.device)] = -1e6

                #create disease mask if model has the option to restrict to diseases
                if hasattr(self, 'restrict_to_diseases') and self.restrict_to_diseases:
                    non_disease_idx = (~self.disease_mask).nonzero(as_tuple=True)[0]
                    scores[:, non_disease_idx.to(scores.device)] = -1e6
