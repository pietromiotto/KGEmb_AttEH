...
# MURP hyp_dist function (which is elemnt wise)
def hyp_distance_multi_c_elementwise(x, y, c):
    """Elementwise Hyperbolic distance on Poincare balls with varying curvatures c.

    Args:
        x: torch.Tensor of size [..., d] with hyperbolic queries
        y: torch.Tensor of size [..., d] with hyperbolic queries
        c: torch.Tensor of size [..., 1] with absolute hyperbolic curvatures

    Return: torch.Tensor with hyperbolic distances, size [..., 1]
    """
    sqrt_c = c ** 0.5
    x2 = torch.sum(x * x, dim=-1, keepdim=True)
    y2 = torch.sum(y * y, dim=-1, keepdim=True)
    ...
    c1 = 1 - 2 * c * xy + c * y2
    c2 = 1 - c * x2
    num_sq = (c1 ** 2) * x2 + (c2 ** 2) * y2 - (2 * c1 * c2) * xy
    num = torch.sqrt(num_sq.clamp_min(MIN_NORM))
    denom = 1 - 2 * c * xy + c ** 2 * x2 * y2
    pairwise_norm = num / denom.clamp_min(MIN_NORM)
    dist = artanh(sqrt_c * pairwise_norm)
    return 2 * dist / sqrt_c
